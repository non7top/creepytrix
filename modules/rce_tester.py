...
class BitrixRCETester:
    ...
    # Generic (non-CVE) admin RCE vectors. Real, verified CVEs now live one file
    # per CVE under modules/cves/ and are loaded via the plugin registry (see
    # _test_known_cves). The two former hardcoded "CVE" entries were removed:
    # the CVE-2022-27228 mapping pointed at the wrong endpoint (correct one is
    # /bitrix/tools/vote/uf.php -- see modules/cves/cve_2022_27228.py), and
    # CVE-2023-28447 is a Smarty XSS, not a Bitrix RCE.
    GENERIC_RCE_ENDPOINTS = [
        {
    ...
    def _test_known_cves(self, base_url: str, result: RCEResult):
        """Test generic admin RCE vectors, then run the per-CVE plugin registry."""
        # 1. Generic (non-CVE) admin vectors kept inline.
        for cve in self.GENERIC_RCE_ENDPOINTS:
            try:
                url = urljoin(base_url, cve['url'])
                resp = self._send_payload(url, cve['method'], cve['payload'])
                if not resp:
                    continue
                if cve['check'] in resp.text:
                    payload = cve['payload']
                    parameter = next(iter(payload.keys()), 'body') if isinstance(payload, dict) else 'body'
                    finding = RCEFinding(
                        severity='critical',
                        rce_type='known_cve',
                        url=url,
                        ...
                        description=f"RCE vector: {cve['name']}",
                        evidence=resp.text[:200]
                    )
                    result.add_finding(finding)
                    self.logger.critical(f"!!! RCE VECTOR: {cve['name']} at {url}")
            except Exception as e:
                self.logger.debug(f"RCE vector error for {cve['name']}: {e}")

        # 2. Real CVEs -- one plugin per CVE under modules/cves/.
        try:
        ...
        for plugin in load_plugins():
            try:
                outcome = plugin.check(self.requester, base_url)
            except Exception as e:
                self.logger.debug(f"CVE plugin error for {plugin.cve_id}: {e}")
                continue
            if not outcome or not outcome.detected:
                continue
            ...
            finding = RCEFinding(
                severity=severity,
                rce_type='known_cve',
                url=urljoin(base_url, plugin.endpoint or ''),
                parameter=f"auth:{plugin.auth}",
                payload=str(plugin.payload)[:100] if plugin.payload else '',
                description=(f"{plugin.cve_id}{disputed} ({outcome.confidence}): "
                             f"{plugin.title}. {outcome.detail}"),
